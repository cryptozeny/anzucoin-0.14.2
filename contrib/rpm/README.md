...
The RPM spec file provided here is for Anzucoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://anzucoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Anzucoin git repository](https://github.com/anzucoin/anzucoin) but are not part of
...
Currently only `Source10` is used, for the example `anzucoin.conf` file.
...
in the anzucoin git repository. At present only an SVG version of the Anzucoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the anzucoin
...
needed when 0.12.1 is released, a proper fix is already in the Anzucoin git
master branch.
...
idea to build Anzucoin Core against a static version of that library compiled
...
The `anzucoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `anzucoin-server` package. At this point, most RPM based
...
The `anzucoin-server` package should have SELinux support. How to properly do
...
Anzucoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/anzucoin/). He has been
packaging Anzucoin for Fedora at least since 2012.
...
4. I renamed the `anzucoin` package that contains the Qt GUI to `anzucoin-core` as
that appears to be how the general population refers to it, in contrast to
`anzucoin-xt` or `anzucoin-classic`. I wanted to make sure the general population

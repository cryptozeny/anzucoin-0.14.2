...
void ImportAddress(const CAnzucoinAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CAnzucoinAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CAnzucoinAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CAnzucoinAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    return CAnzucoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CAnzucoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CAnzucoinSecret(key).ToString(), strTime);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CAnzucoinAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CAnzucoinAddress redeemAddress = CAnzucoinAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CAnzucoinAddress pubKeyAddress = CAnzucoinAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CAnzucoinAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CAnzucoinAddress(destination);
